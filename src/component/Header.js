      ...
      <nav className="justify-between items-center flex   ">
        <div className="md:flex top-0 left-0  z-50  md:z-50  w-52 md:w-72 md:mr-96 flex flex-row ">
          <Tooltip title={t("Back to Home")}>
            <div className="flex justify-center flex-row text-center items-center gap-x-2">
              ...
              <a
                href="/"
                className="hover:animate-ping font-black flex justify-center flex-row text-center mt-2
        ...
        <ul
          ...
          className={`
            ...
            fixed z-20 top-0 left-0 w-full h-full flex font-bold flex-col justify-center items-center  bg-opacity-80 ${
              menuVisible ? "min-h-full" : "invisible"
            } md:visible md:bg-transparent md:h-auto md:flex-row md:justify-between md:static`}
        >
          <li className="m-8 md:m-0">
            <a
              href="/"
              className=" md:text-white hover:animate-pulse hover:border-b-2 hover:border-red-500 hover:text-red-500 transition duration-500 ease-in"
            >
              {t("Home")}
            </a>
          </li>
          ...
          <li className="m-8 md:m-0"></li>
        </ul>
        <div
          id="menu-button"
          className="absolute  pt-5 top-5 left-5  md:hidden cursor-pointer z-50 flexd"
          onClick={toggleMenu}
        >
          <FloatButton
            icon={<MenuOutlined style={{ color: "black" }} />}
            className={`flex z-50  top-3 right-5 bg-`}
          />
        </div>
      </nav>
    </div>
